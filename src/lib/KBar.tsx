    ...
    return <KBarPortal>
        <KBarPositioner
            ...
            <KBarAnimator className='font-content bg-slate-800 rounded-md shadow-xl flex flex-col space-x-4 w-[35rem] overflow-hidden'>
                <div className='flex px-2 font-medium text-slate-500 focus-within:text-slate-200 selection:bg-white/20'>
                ...
                <div className='flex flex-col'>
                    ...
                    {results.length != 0 && <div className='animate-faceIn border-t-2 border-slate-700/50 pb-3 pt-2'>
                        <KBarResults
                            ...
                            onRender={({ item, active }) =>
                                typeof item == 'string' ? (
                                    // section heading
                                    <div className='text-xs pt-3 pb-1 px-[34px] text-slate-500 font-bold uppercase'>
                                        {item}
                                    </div>
                                ) : (
                                    // each item
                                    <div className='flex flex-col py-1'>
                                        <div className='flex flex-col px-4'>
                                            <div
                                                className={`flex pl-4 py-3 transition-colors rounded-md ${active ? 'bg-slate-600/70 cursor-pointer' : 'bg-slate-700/30'}`}>
                                                {/* the icon */}
                                                {item.icon && <div
                    ...
                    {/* when no results */}
                    {results.length == 0 && <div className='animate-faceIn select-none pointer-events-none  text-slate-400 flex flex-col justify-center items-center pt-8 pb-12 space-y-4'>
